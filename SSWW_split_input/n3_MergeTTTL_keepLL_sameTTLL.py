#Artifically making comparable amount for LL and TTTL event number
...
class New_tree(object):
    ...
    def Generate(self, tag="testtest",TTTL_Multi=1):
        ...
        TTTL_flag = 0; LL_flag = 0
        #LL_list = []
        for i in range(Entry):
            if i%10000==0: print("[picking LL]Now looping", i," of Total",Entry);  
            tree.GetEntry(i)
            if(LL_Helicity[0] == 1.0): TTTL_Helicity[0] = 0.0; tree_w.Fill(); LL_flag += 1; #LL_list.append(i)
            else:  continue
        TTTL_Max = TTTL_Multi * LL_flag

        for i in range(Entry):
            if i%10000==0: print("[picking LL]Now looping", i," of Total",Entry);
            tree.GetEntry(i)
            if( (TTTL_flag<=TTTL_Max) & (LL_Helicity[0] == 0.0)  ): 
                TTTL_Helicity[0] = 1.0
                TTTL_flag += 1
                tree_w.Fill();
...
def main():
    infile = "/Users/leejunho/Desktop/git/PKUHEP/DNN/SSWW_input/SS_4p0M.root"
    #infile = "/Users/leejunho/Desktop/git/PKUHEP/DNN/SSWW_input/small_test_Ntuple.root"
    Test = New_tree(infile)
    Test.Generate(tag="TTTL_LL_comparable", TTTL_Multi = 1) #FIXME TTTL_Multi 
