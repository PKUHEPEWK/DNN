...
for i in range(len(LL_TTTL_prob_tuple[2])):
    #if(i>3000): break
    a = (lep1pt[i], lep1eta[i], lep1phi[i], lep2pt[i], lep2eta[i], lep2phi[i], jet1pt[i], jet1eta[i], jet1phi[i], jet1M[i], jet2pt[i], jet2eta[i], jet2phi[i], jet2M[i], MET[i], lep1PID[i], lep2PID[i], Mjj[i], dr_ll_jj[i], dphijj[i], zeppen_lep1[i], zeppen_lep2[i], METphi[i], detajj[i], Mll[i], RpT[i])
...
('METphi',np.float32),('detajj',np.float32),('Mll',np.float32),('RpT',np.float32)] )
...
LL_file = TFile("SSWW_classification_predict/LL_predict.root","RECREATE")
try:
...
TTTL_file = TFile("SSWW_classification_predict/TTTL_predict.root","RECREATE")
try:
