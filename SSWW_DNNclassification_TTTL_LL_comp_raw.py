...
model = DNN(n_in=16, n_hiddens=[150,150,150,150,150,150,150,150,150,150], n_out=2) #TODO FIXME
#epochs = 1000
epochs = 300
...
Model_name = "Raw_"+str(Date)+"_"+"TrainENum"+str(N_train)+"/"+"LayerNum_"+str(Layer_NUM)+"+"+"Node_"+str(Node_on_Each_layer)+"+"+"BatchSize_"+str(batch_size)
Make_dir = "mkdir -p "+ "tens_model_class/"+Model_name
os.system(Make_dir)
model_name = Model_name +"/"+ "SSWW_tensor_TTTL-LL_comp"

data = TFile.Open('/sps/cms/jlee/DNN/SSWW_split_input/result/for_DNN/TTTL_LL_4p0M_comparable_2.root')
...
('MET',np.float32),('METphi',np.float32),('LL_Helicity',np.float32), ('TTTL_Helicity',np.float32)] )
#print(TEST_nplist)
